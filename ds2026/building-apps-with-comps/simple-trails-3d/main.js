async function load() {
    ...
    reactiveUtils.watch(
        ...
        (progress) => {
            if (progress === 1) {
                ...
                const elevGain = Math.round((analysisView.statistics.elevationGain + Number.EPSILON) * 100) / 100;
                const distance = Math.round((analysisView.statistics.maxDistance + Number.EPSILON) * 100) / 100;
                elevationElem.innerText = elevGain + " " + elevProf.effectiveDisplayUnits.elevation;
                distanceElem.innerText = distance + " " + elevProf.effectiveDisplayUnits.distance;
            }
        }
    );
    ...
    mapElem.addEventListener("arcgisViewClick", async (event) => {
        const { results } = await view.hitTest(event.detail, { include: [trailsLayer] })
        if (results?.length > 0) {
    ...
    view.selectionManager.on("selection-change", async (event) => {
        const results = await view.selectionManager.getSelectedFeatures([trailsLayer], { returnGeometry: true });
